...
const BOOT_MANIFEST_HASH_FIXED='MANIFEST_HASH="2f2212c7e6a91b4d"';
...
const BOOT_WARM_FIXED="{const homeEnd=Math.max(0,SCRIPT_ORDER.indexOf('js/player-journey-v2110.js'));return MANIFEST.filter(a=>a.k==='style'||ENTRY_VISUALS.includes(a.u)||(a.k==='script'&&SCRIPT_ORDER.indexOf(a.u)>=0&&SCRIPT_ORDER.indexOf(a.u)<=homeEnd)).every(a=>have.has(a.u))}";
const BOOT_QUEUE_BAD='const q=[...MANIFEST],concurrency=';
const BOOT_QUEUE_FIXED="const homeEnd=Math.max(0,SCRIPT_ORDER.indexOf('js/player-journey-v2110.js')),q=MANIFEST.filter(a=>a.k==='style'||ENTRY_VISUALS.includes(a.u)||(a.k==='script'&&SCRIPT_ORDER.indexOf(a.u)>=0&&SCRIPT_ORDER.indexOf(a.u)<=homeEnd));state.totalFiles=q.length;const concurrency=";
...
const BOOT_CORE_FIXED="let bootIndex=0;const homeReadyIndex=Math.max(0,SCRIPT_ORDER.indexOf('js/player-journey-v2110.js'));for(;bootIndex<SCRIPT_ORDER.length;bootIndex++){await runScript(SCRIPT_ORDER[bootIndex],cache,bootIndex);if(bootIndex>=homeReadyIndex&&(window.__POWDER_APP_BOOTED__||window.POWDER_APP)){appReadyFlag=true;bootIndex++;break}}scriptsDone=true;if(window.__POWDER_APP_BOOTED__||window.POWDER_APP)appReadyFlag=true;const deferredFrom=bootIndex;setTimeout(async()=>{while(!entered)await sleep(25);await sleep(280);for(let j=deferredFrom;j<SCRIPT_ORDER.length;j++){try{await runScript(SCRIPT_ORDER[j],cache,j)}catch(e){console.error('[Powder post-boot module]',SCRIPT_ORDER[j],e)}}try{window.dispatchEvent(new CustomEvent('powder:post-boot-complete',{detail:{from:deferredFrom,total:SCRIPT_ORDER.length}}))}catch(_){}},0);";
