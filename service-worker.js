...
async function repairBuildResponse(url,response){if(!response?.ok)return response;const domain=url.pathname.endsWith('/js/domain-system-v15.js'),auth=url.pathname.endsWith('/js/online-foundation-v150.js');if(!domain&&!auth)return response;try{const text=await response.text(),fixed=domain?text.replace(DOMAIN_LOCK_BAD,DOMAIN_LOCK_FIXED):text.replace(AUTH_REFRESH_BAD,AUTH_REFRESH_FIXED),headers=new Headers(response.headers);headers.delete('content-length');headers.delete('content-encoding');headers.set('x-powder-runtime-repair',domain?'domain-lock-2136':'auth-refresh-2136');return new Response(fixed,{status:response.status,statusText:response.statusText,headers})}catch{return response}}
