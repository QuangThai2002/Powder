...
export class CombatPresentationDirector {
  ...
  async playSkillIntro(
    ...
    await Promise.all([
      this.tween({ targets: fx, scaleX: 1.22, scaleY: 1.22, alpha: 0, duration: this.reducedMotion ? 120 : slot === 0 ? 210 : 260, ease: 'Quad.easeOut' }),
      this.tween({ targets: path, alpha: 0, duration: this.reducedMotion ? 130 : 280, ease: 'Quad.easeOut' })
    ]);
    fx.destroy(true);
    path.destroy();
  }
  ...
  async playUltimateIntro(actorView: PowView | undefined, ability: CombatAbility, side: CombatSide, elementKey: string): Promise<void> {
    ...
    const actor = actorView.getWorldPosition();
    const element = this.elementColor(elementKey);
    const overlay = this.scene.add.rectangle(width / 2, height / 2, width, height, 0x02070d, 0.48).setDepth(70).setAlpha(0);
    const spot = this.scene.add.circle(actor.x, actor.y, 80, element, 0.06).setStrokeStyle(5, element, 0.92).setDepth(72).setScale(0.7);

    const bannerY = Math.round(height * 0.43);
    const bannerWidth = Math.min(650, width * 0.47);
    const banner = this.scene.add.container(width / 2, bannerY).setDepth(76).setAlpha(0).setScale(0.96);
    const plate = this.scene.add.rectangle(0, 0, bannerWidth, 116, 0x07131d, 0.98).setStrokeStyle(3, 0xffd36a, 0.9);
    const rail = this.scene.add.rectangle(side === 'player' ? -bannerWidth / 2 + 5 : bannerWidth / 2 - 5, 0, 8, 104, element, 0.95);
    const iconX = -bannerWidth / 2 + 68;
    const iconPlate = this.scene.add.rectangle(iconX, 0, 82, 82, 0x030a10, 0.95).setStrokeStyle(2, element, 0.76);
    const icon = ability.iconKey && this.scene.textures.exists(ability.iconKey)
      ? this.scene.add.image(iconX, 0, ability.iconKey).setDisplaySize(74, 74)
      : this.scene.add.text(iconX, 0, '✦', { fontFamily: COMBAT_DISPLAY_FONT, fontSize: '42px', color: '#ffd36a' }).setOrigin(0.5);
    const textX = -bannerWidth / 2 + 124;
    const label = this.scene.add.text(textX, -34, 'TUYỆT KỸ', {
      fontFamily: COMBAT_DISPLAY_FONT, fontSize: '16px', color: '#ffd36a', fontStyle: 'bold', letterSpacing: 2
    }).setOrigin(0, 0.5);
    const name = this.scene.add.text(textX, 2, this.shortName(ability.name, 34).toUpperCase(), {
      fontFamily: COMBAT_DISPLAY_FONT, fontSize: '25px', color: '#fff8e7', fontStyle: 'bold', fixedWidth: Math.max(280, bannerWidth - 150)
    }).setOrigin(0, 0.5);
    const sub = this.scene.add.text(textX, 37, side === 'player' ? 'POW CỦA BẠN' : 'POW ĐỐI THỦ', {
      fontFamily: COMBAT_BODY_FONT, fontSize: '12px', color: side === 'player' ? '#8eeaff' : '#ffb18d', fontStyle: 'bold'
    }).setOrigin(0, 0.5);
    banner.add([plate, rail, iconPlate, icon, label, name, sub]);

    if (!this.reducedMotion) this.scene.cameras.main.flash(120, 255, 224, 145, false);

    // Readability contract: ~0.25s reveal + 1.25s hold + 0.28s exit.
    await Promise.all([
      this.tween({ targets: overlay, alpha: 1, duration: 220, yoyo: true, hold: this.reducedMotion ? 450 : 1280, ease: 'Sine.easeOut' }),
      this.tween({ targets: spot, scaleX: 1.28, scaleY: 1.28, alpha: 0, duration: this.reducedMotion ? 650 : 1600, ease: 'Quad.easeOut' }),
      this.tween({ targets: banner, alpha: 1, scaleX: 1, scaleY: 1, duration: 240, yoyo: true, hold: this.reducedMotion ? 420 : 1250, ease: 'Quad.easeOut' })
    ]);
  ...
  async playUltimateImpact(targetView: PowView | undefined, elementKey: string, selfTargeted: boolean): Promise<void> {
    ...
    await this.tween({ targets: burst, scaleX: 1.8, scaleY: 1.8, alpha: 0, duration: this.reducedMotion ? 150 : 260, ease: 'Quad.easeOut' });
    burst.destroy(true);
  }
  ...
  showElementOutcome(view: PowView | undefined, outcome: ElementOutcome): void {
    ...
    const text = this.scene.add.text(p.x, y, item.label, {
      fontFamily: COMBAT_DISPLAY_FONT, fontSize: '16px', color: item.color, fontStyle: 'bold', backgroundColor: '#06111cdd', padding: { x: 10, y: 5 }, stroke: '#02070d', strokeThickness: 2
    }).setOrigin(0.5).setDepth(55);
    this.scene.tweens.add({ targets: text, y: y + (p.y < this.scene.scale.height / 2 ? 10 : -10), alpha: 0, duration: this.reducedMotion ? 500 : 800, ease: 'Quad.easeOut', onComplete: () => text.destroy() });
  }
