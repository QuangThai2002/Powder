...
const PATCH_FLAG = '__powderCombat295VersionInstalled';

export function installCombat295VersionPatch(BattleSceneClass: any): void {
  const root = globalThis as any;
  if (root[PATCH_FLAG]) return;
  root[PATCH_FLAG] = true;
  const proto = BattleSceneClass.prototype as any;

  proto.showPreBattleIntro = function combat295Intro(this: PatchableScene): void {
    ...
    const title = this.add.text(width / 2, height / 2, 'POWDER COMBAT 2.9.5', {
  ...
  if (api && typeof api === 'object') api.version = '2.9.5';
}
