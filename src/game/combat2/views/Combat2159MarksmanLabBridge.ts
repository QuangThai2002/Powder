...
import {
  COMBAT2159_MARKSMAN_VERSION,
  playCombat2159MarksmanDirectVfx
} from '../vfx/Combat2159MarksmanDirectVfx';

const FLAG = '__powderCombat2159MarksmanLabBridgeInstalled';
...
function installCombat2159MarksmanLabBridge(): void {
  ...
  if (!api || typeof api.play !== 'function') {
    root.POWDER_COMBAT2_2159_MARKSMAN_LAB = {
  ...
  api.play = async (requestedRole?: string | null, requestedElement?: CombatProjectileElement | null) => {
    ...
    try {
      ...
      const result = {
        ...
        directVersion: COMBAT2159_MARKSMAN_VERSION,
      ...
      console.error('[Combat2 2.15.9 Marksman Direct Lab]', error);
      return {
        ok: false,
        reason: 'direct-marksman-vfx-threw',
        role: 'marksman',
        element,
        directVersion: COMBAT2159_MARKSMAN_VERSION
      };
...
installCombat2159MarksmanLabBridge();
