...
const transport={
 async begin(payload={}){const s=server();if(!s?.domainClashBegin)throw new Error('Server Combat chưa hỗ trợ domain_clash_begin.');st.begins++;st.lastAt=Date.now();try{const r=await s.domainClashBegin({...payload,questionCount:10,questionMs:5000,learningRuleVersion:'21.6.8',requireVerifiedLearning:true});return r?.clash||r}catch(e){st.lastError=String(e?.message||e);throw e}},
...
function snapshot(){return{version:VERSION,...st,serverReady:!!server()?.domainClashBegin,clashReady:!!clash()?.registerTransport,rules:{questionCount:10,questionMs:5000,serverAuthoritative:true,clientSelfResolution:false,learningRuleVersion:'21.6.8',verifiedLearningRequired:true},serverStatus:server()?.domainClashAuthorityStatus?.()||null}}
