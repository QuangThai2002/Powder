...
const VERSION='22.1.2-combat-runtime-integrity',root=document.documentElement;
...
function collect(reason='event'){
  ...
  root.dataset.performanceDirector=state.status;root.dataset.performanceGoalFps=String(Math.round(hz));root.dataset.performanceHeadroom=String(state.headroom);root.dataset.combatPipeline='identity-2212';
...
function loadCombatRuntimeQaBridge(){return addScript('powderCombatRuntimeQaBridge2140','js/combat-runtime-qa-bridge-v2140.js?v=2212','POWDER_COMBAT_RUNTIME_QA_BRIDGE_V2140')}
...
function activateTurnSafety(){return addScript('powderCombatTurnSafety2207','js/combat-turn-safety-v2207.js?v=2212','POWDER_COMBAT_TURN_SAFETY_V2207')}
function activateStableCombatLayout(){return addScript('powderCombatLayout2200','js/combat-layout-v2200.js?v=2212','POWDER_COMBAT_LAYOUT_V2200')}
function activateStablePlayerExperience(){return addScript('powderCombatPlayerExperience2201','js/combat-player-experience-v2201.js?v=2212','POWDER_COMBAT_PLAYER_EXPERIENCE_V2201')}
function activateRealPlayerBaseline(){return addScript('powderCombatRealPlayerBaseline2205','js/combat-real-player-baseline-v2205.js?v=2212','POWDER_COMBAT_REAL_PLAYER_BASELINE_V2205')}
function activateAncientExclusiveAltar(){return addScript('powderCombatExclusiveAltar2206','js/combat-exclusive-altar-v2206.js?v=2212','POWDER_COMBAT_EXCLUSIVE_ALTAR_V2206')}
function activateCombatIdentityBreakthrough(){return addScript('powderCombatIdentityBreakthrough2210','js/combat-identity-breakthrough-v2210.js?v=2212','POWDER_COMBAT_IDENTITY_BREAKTHROUGH_V2210')}
function activateCombatCommandFeedback(){return addScript('powderCombatCommandFeedback2212','js/combat-command-feedback-v2212.js?v=2212','POWDER_COMBAT_COMMAND_FEEDBACK_V2212')}
...
function boot(){
  ...
  activateStableCombatLayout();activateStablePlayerExperience();activateRealPlayerBaseline();activateAncientExclusiveAltar();activateCombatIdentityBreakthrough();activateCombatCommandFeedback();
  state.stableCombatBoots++;state.lastAt=Date.now();state.lastReason='combat-runtime-2212-boot';
}
function snapshot(){return{version:VERSION,...state,pressure:pressure(),highRefresh:hs(),frameBudget:fb(),combatGovernor:cg(),scrollPipeline:sp(),runtimeRecovery:window.POWDER_RUNTIME_RECOVERY_V2136?.snapshot?.()||null,combatActionClarity:window.POWDER_COMBAT_ACTION_CLARITY_V2138?.snapshot?.()||null,domainExportRecovery:window.POWDER_DOMAIN_EXPORT_RECOVERY_V2140?.snapshot?.()||null,combatRuntimeQaBridge:window.POWDER_COMBAT_RUNTIME_QA_BRIDGE_V2140?.snapshot?.()||null,turnSafety:window.POWDER_COMBAT_TURN_SAFETY_V2207?.snapshot?.()||null,layout:window.POWDER_COMBAT_LAYOUT_V2200?.snapshot?.()||null,playerExperience:window.POWDER_COMBAT_PLAYER_EXPERIENCE_V2201?.snapshot?.()||null,realPlayerBaseline:window.POWDER_COMBAT_REAL_PLAYER_BASELINE_V2205?.snapshot?.()||null,ancientExclusiveAltar:window.POWDER_COMBAT_EXCLUSIVE_ALTAR_V2206?.snapshot?.()||null,combatIdentity:window.POWDER_COMBAT_IDENTITY_BREAKTHROUGH_V2210?.snapshot?.()||null,commandFeedback:window.POWDER_COMBAT_COMMAND_FEEDBACK_V2212?.snapshot?.()||null,disabledPresentationModules:[...DISABLED_FX],pipeline:'21.4 stable Combat Scene/Core + 22.1.2 finite scheduler hardening + single-owner pooled identity + locked-skill feedback',policy:'Legacy post-21.4 decorative chain stays disabled. 22.1.2 centralizes Identity boot ownership, aligns runtime cache versions and adds finite player feedback for unavailable skills; no gameplay, damage, skill-data, server or save mutation.'}}
