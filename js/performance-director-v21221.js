...
const VERSION='22.0.6-stable-combat',root=document.documentElement;
...
function collect(reason='event'){
  ...
  root.dataset.performanceDirector=state.status;root.dataset.performanceGoalFps=String(Math.round(hz));root.dataset.performanceHeadroom=String(state.headroom);root.dataset.combatPipeline='stable-2206';
...
function loadCombatRuntimeQaBridge(){return addScript('powderCombatRuntimeQaBridge2140','js/combat-runtime-qa-bridge-v2140.js?v=2205','POWDER_COMBAT_RUNTIME_QA_BRIDGE_V2140')}
...
function activateStableCombatLayout(){return addScript('powderCombatLayout2200','js/combat-layout-v2200.js?v=2205','POWDER_COMBAT_LAYOUT_V2200')}
function activateStablePlayerExperience(){return addScript('powderCombatPlayerExperience2201','js/combat-player-experience-v2201.js?v=2205','POWDER_COMBAT_PLAYER_EXPERIENCE_V2201')}
function activateRealPlayerBaseline(){return addScript('powderCombatRealPlayerBaseline2205','js/combat-real-player-baseline-v2205.js?v=2205','POWDER_COMBAT_REAL_PLAYER_BASELINE_V2205')}
function activateAncientExclusiveAltar(){return addScript('powderCombatExclusiveAltar2206','js/combat-exclusive-altar-v2206.js?v=2206','POWDER_COMBAT_EXCLUSIVE_ALTAR_V2206')}
...
function boot(){
  collect('boot');
  activateCombatFoundation();activateDomainMenuGuard();activateRuntimeRecovery();activateCombatActionClarity();activateCombatRuntimeQaBridge();
  activateStableCombatLayout();activateStablePlayerExperience();activateRealPlayerBaseline();activateAncientExclusiveAltar();
  state.stableCombatBoots++;state.lastAt=Date.now();state.lastReason='stable-combat-boot';
}
function snapshot(){return{version:VERSION,...state,pressure:pressure(),highRefresh:hs(),frameBudget:fb(),combatGovernor:cg(),scrollPipeline:sp(),runtimeRecovery:window.POWDER_RUNTIME_RECOVERY_V2136?.snapshot?.()||null,combatActionClarity:window.POWDER_COMBAT_ACTION_CLARITY_V2138?.snapshot?.()||null,domainExportRecovery:window.POWDER_DOMAIN_EXPORT_RECOVERY_V2140?.snapshot?.()||null,combatRuntimeQaBridge:window.POWDER_COMBAT_RUNTIME_QA_BRIDGE_V2140?.snapshot?.()||null,layout:window.POWDER_COMBAT_LAYOUT_V2200?.snapshot?.()||null,playerExperience:window.POWDER_COMBAT_PLAYER_EXPERIENCE_V2201?.snapshot?.()||null,realPlayerBaseline:window.POWDER_COMBAT_REAL_PLAYER_BASELINE_V2205?.snapshot?.()||null,ancientExclusiveAltar:window.POWDER_COMBAT_EXCLUSIVE_ALTAR_V2206?.snapshot?.()||null,disabledPresentationModules:[...DISABLED_FX],pipeline:'21.4 stable Combat Scene/Core + 22.0.6 layout + Ancient exclusive altar',policy:'No post-21.4 decorative Combat chain auto-boots. Re-enable modules only after isolated regression testing.'}}
...
window.POWDER_PERFORMANCE_DIRECTOR_V21221={version:VERSION,snapshot,refresh:()=>collect('manual'),remeasure:()=>requestRemeasure('manual'),activateCombatFoundation,activateDomainMenuGuard,activateRuntimeRecovery,activateCombatActionClarity,activateCombatRuntimeQaBridge,activateStableCombatLayout,activateStablePlayerExperience,activateRealPlayerBaseline,activateAncientExclusiveAltar,activateCombatFxFidelity,activateCombatRoleRhythm,activateCombatImpactSequence,activateCombatImpactLegibility,activateCombatStatusFx,activateCombatCameraHit,activateCombatUltimateIdentity,activateCombatFxBudget,activateCombatMagicCircle,activateCombatGrandMagic,activateCombatSpellChain,activateCombatMagicFinish,activateCombatSpellProjectile,activateCombatElementalClash,activateCombatBarrierMagic,activateCombatMagicSceneDirector,activateCombatCastChoreography,activateCombatElementalField};
