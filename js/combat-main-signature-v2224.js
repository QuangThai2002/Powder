...
let mount=null,sigil=null,observer=null;
...
function attach(next){if(next===mount)return;cleanup(mount);observer?.disconnect();mount=next;sigil=null;state.active=!!mount;if(!mount)return;mount.classList.add('cms224-ready');observer=new MutationObserver(()=>sync());observer.observe(mount,{subtree:true,childList:true,attributes:true,attributeFilter:['hidden','class']});sync()}
...
function sync(){const next=pickMount();if(next!==mount){attach(next);return}if(!mount)return;state.active=true;mount.classList.add('cms224-ready');mount.classList.toggle('cms224-pressure-reduced',pressure()==='hot'||pressure()==='critical')}
function start(){ensureCss();attach(pickMount());window.addEventListener('powder:combat-skill-identity',e=>play(e.detail||{}),{passive:true});window.addEventListener('powder:combat-presentation-sync',sync,{passive:true});window.addEventListener('powder:resource-pressure',sync,{passive:true});const bodyObserver=new MutationObserver(()=>sync());bodyObserver.observe(document.body,{subtree:true,childList:true,attributes:true,attributeFilter:['hidden']})}
