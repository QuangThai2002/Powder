...
function createBossPilotRequest(stage){const player=playerTeam(),challengeId=String(stage?.bossChallengeId||'').toLowerCase(),enemy=ids(stage?.enemyIds).slice(0,1),config=bossConfig(challengeId),save=window.POWDER_APP?.getSave?.()||{};return createBattleRequest({battleMode:'boss',sourceContext:{kind:'boss',stage:clone(stage),stageId:String(stage?.id||''),bossChallengeId:challengeId},returnContext:{view:'boss',bossChallengeId:challengeId},playerTeam:player,formation:{active:player.slice(0,3),reserve:player.slice(3,5)},enemyTeam:enemy,enemyConfig:{bossId:enemy[0]||null,difficulty:String(stage?.difficultyLabel||'BOSS')},battleRules:{stageScale:Number(stage?.scale)||1,initiative:Number(stage?.initiative)||0,manaStart:Number(stage?.manaStart)||0,rageStart:Number(stage?.rageStart)||0},bossContext:{bossChallengeId:challengeId,bossType:challengeId,bossId:enemy[0]||null,rank:Number(save.rank)||0,difficulty:String(stage?.difficultyLabel||'BOSS'),phaseConfig:config,rewardContext:{claimId:String(stage?.id||challengeId),rewardEligible:stage?.rewardEligibleServer!==false},cooldownContext:{challengeId}},academicContext:{authority:'boss-qualified-main',requiresActionQuestions:false},rewardContext:{rewardId:`boss:${challengeId}`}});}
function canRunBossPilot(request){return !!request?.ok&&request.value.battleMode==='boss'&&!!request.value.bossContext?.phaseConfig&&typeof window.POWDER_APP?.onBossCombatFinished==='function'&&typeof window.POWDER_TX_SAFETY_V2090?.mutate==='function'&&!window.POWDER_ONLINE_V150?.hasSession?.()&&!request.value.sourceContext?.stage?.serverCombatSessionId;}
...
function startBoss(stage){const s=normalize(stage,'boss');if(!s||s.kind!=='boss'||!s.bossChallengeId)return false;/* Combat2 Boss remains gated until legacy adaptive roster/resource bootstrap has parity. */return !!scene()?.startEncounter?.(s,'boss');}
