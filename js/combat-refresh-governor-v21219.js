...
const state={cap:'high',displayHz:60,currentFps:60,ratio:1,pressure:'calm',updates:0,lastAt:0};let timer=0;
...
function onView(e){apply('view');const v=String(e?.detail?.view||document.body?.dataset?.activeView||'');if(/battle|boss|combat/i.test(v)){schedule('battle-settle',500);setTimeout(()=>{if(!document.hidden)apply('battle-followup')},1800)}}
function snapshot(){return{version:VERSION,...state,preservedFeedback:['damage','heal','shield','stun','freeze','skip-turn','core hit reaction'],policy:'degrade decorative combat FX only when native refresh headroom is insufficient'}}
function activateNext(){if(window.POWDER_SCROLL_PIPELINE_V21220||document.getElementById('powderScrollPipeline21220'))return;const s=document.createElement('script');s.id='powderScrollPipeline21220';s.src='js/scroll-pipeline-v21220.js?v=21220';s.async=true;document.head.appendChild(s)}
function boot(){style();apply('boot');activateNext()}
window.addEventListener('powder:high-refresh-profile',()=>apply('high-refresh'),{passive:true});window.addEventListener('powder:frame-budget',()=>apply('frame-budget'),{passive:true});window.addEventListener('powder:resource-pressure',()=>apply('pressure'),{passive:true});window.addEventListener('powder:view-changed',onView,{passive:true});document.addEventListener('visibilitychange',()=>{if(document.hidden){clearTimeout(timer);timer=0}else apply('visibility')},{passive:true});window.addEventListener('pagehide',()=>{clearTimeout(timer);timer=0},{once:true});
