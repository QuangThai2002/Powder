...
function diagnostics(){const D=window.POWDER_DATA||{},W=window.POWDER_WORLD_DATA_V9||{},S=window.POWDER_SKILL_V81||{},L=window.POWDER_LEARNING_MASTER_V2||{},A=window.POWDER_ADVENTURE_DATA||{},Core=window.POWDER_COMBAT_CORE_V7||{},pows=D.pows||[],kits=S.pows||{},skills=[];for(const k of Object.values(kits))for(const slot of ['basic','skill1','skill2','ultimate'])if(k?.skills?.[slot]?.id)skills.push(String(k.skills[slot].id));const q=L.activeQuestions?.(D.questions||[])||D.questions||[],badQuestions=q.filter(x=>!x?.id||x.answer==null||!Array.isArray(x.options)||x.options.length<2||!x.options.map(String).includes(String(x.answer))),rarities=['common','rare','super_rare','epic','legendary','mythic','ancient'],artifactCounts=Object.fromEntries(rarities.map(r=>[r,(W.artifacts||[]).filter(x=>x.rarity===r).length])),skillApi=window.POWDER_SKILL_LOADOUT,domainApi=window.POWDER_PROGRESSION_GATE_V1782,domainCore=Core.BattleCore?.prototype,gearBoard=document.querySelector('#gearView .pow-skill-board'),gearSlots=gearBoard?[...gearBoard.querySelectorAll('.pow-skill-slot')].map(x=>[...x.classList].find(c=>c.startsWith('slot-kind-'))):[],requiredSlots=['basic','skill1','skill2','ultimate'],combatPresentation=window.POWDER_COMBAT_PRESENTATION_V21015?.snapshot?.()||{},combatContent=window.POWDER_COMBAT_CONTENT_UI_V1890,learningHub=window.POWDER_LEARNING_HUB_V149;
 const checks={
  ...
  learningRuntimeConsolidated:String(window.POWDER_SRS_INTELLIGENCE_V2114?.runtimePolicy||'').includes('logic-only')&&!window.POWDER_LEARNING_RUNTIME_RECOVERY_V21210,
  ...
  equipment:(W.equipmentSets||[]).length===13&&(W.equipment||[]).length===52,
  ...
  cloudSave:!!window.POWDER_ONLINE_V150,
  performance:String(window.POWDER_ADAPTIVE_PRESSURE_V21011?.mode||'').includes('event-driven'),
 ...
 return{version:VERSION,buildId:BUILD,at:Date.now(),pass:Object.values(checks).every(bool),checks,details:{pows:pows.length,kits:Object.keys(kits).length,skills:skills.length,uniqueSkills:new Set(skills).size,lessons:(D.lessons||[]).length,questions:q.length,badQuestions:badQuestions.slice(0,10).map(x=>x.id||'(missing id)'),islands:(A.islands||[]).length,equipmentSets:(W.equipmentSets||[]).length,equipment:(W.equipment||[]).length,artifacts:(W.artifacts||[]).length,artifactCounts,gearSlots,domainSimple:Object.keys(Core.TAMER_SIMPLE||{}),domainExpansion:Object.keys(Core.TAMER_EXPANSIONS||{}),learningRuntimePolicy:window.POWDER_SRS_INTELLIGENCE_V2114?.runtimePolicy||'',learningHubMode:learningHub?.mode||'',pressureMode:window.POWDER_ADAPTIVE_PRESSURE_V21011?.mode||'',combatPresentationVersion:window.POWDER_COMBAT_PRESENTATION_V21015?.version||'',combatContentVersion:combatContent?.version||''}}}
const api=Object.freeze({version:VERSION,buildId:BUILD,diagnostics,state:()=>clone(diagnostics())});window.POWDER_FULL_GAME_REGRESSION_V20180=api;try{window.dispatchEvent(new CustomEvent('powder:full-game-regression-ready',{detail:diagnostics()}))}catch(_){}
})();
