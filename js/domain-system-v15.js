...
function activeSimple(core,side){return sideState(core,side).simpleActive}
function activeExpansion(core,side){return sideState(core,side).expansion}
...
function consumeExpansionAction(core,side,{skipped=false}={}){const st=sideState(core,side),d=st.expansion;if(!d)return;if(d.id==='draw_swords'&&!skipped)swordStrike(core,side);d.remainingActions=Math.max(0,Number(d.remainingActions||1)-1);if(d.remainingActions<=0)clearExpansion(core,side,'duration')}
...
P.canUseTamerSimple=function(sideOrId,idMaybe,levelMaybe){let side='player',id=idMaybe;if(SIMPLE[sideOrId]){id=sideOrId;side=this.state.current?.side||'player'}else side=sideOrId||'player';const st=sideState(this,side),cur=this.state.current;if(!SIMPLE[id]||!cur||cur.side!==side||this.state.phase!=='running'||this.state.finished||st.simpleCharges<=0||st.expansion||st.simpleActive)return false;return true};
...
P.setPendingDomainActionScale=function(side,scale=1){const st=sideState(this,side),v=clamp(Number(scale)||0,0,3);st.pendingActionScale=clamp(Number(st.pendingActionScale??1)*v,0,3);return st.pendingActionScale};
P.clearPendingDomainActionScale=function(side){sideState(this,side).pendingActionScale=1};
P.addLimitlessCorrect=function(side,count=0){const st=sideState(this,side),d=st.expansion;if(!d||d.id!=='limitless_void')return false;st.limitlessCorrect=Math.min(10,Number(st.limitlessCorrect||0)+Math.max(0,Number(count)||0));if(st.limitlessCorrect>=10&&!st.limitlessBurstDone){st.limitlessBurstDone=true;const src=this.living(side)[0];let total=0;for(const t of this.living(foeSide(side)))total+=domainHpDamage(this,src,t,.50,'Vô Lượng Quá Tải');this.pushLog?.(`Vô Lượng · đủ 10 câu đúng → toàn đội địch chịu 50% Max HP (${total.toLocaleString('vi-VN')} damage).`,'win');this.pushEvent?.('domain-limitless-burst',{side,correct:st.limitlessCorrect,total});return true}return false};
P.start=function(){ensure(this);return oldStart.call(this)};
P.beginTurn=function(){ensure(this);const beforeLen=(this.events||[]).length,beforeStatuses={};for(const u of this.allUnits||[])beforeStatuses[u.id]=Object.fromEntries(Object.entries(u.statuses||{}).map(([k,v])=>[k,{...v}]));const r=oldBegin.call(this);const newEvents=(this.events||[]).slice(beforeLen);processDomainTicks(this,newEvents,beforeStatuses);const cur=this.state.current;if(cur){for(const side of ['player','enemy']){const st=sideState(this,side),a=st.simpleActive;if(a){a.actionsElapsed=Number(a.actionsElapsed||0)+1;const activator=units(this,side,true).find(u=>u.id===a.actorId&&!u.defeated);if(!activator||((cur.id===a.actorId)&&a.hasCompletedCycle)||a.actionsElapsed>12)clearSimple(this,side,'turn')}const d=st.expansion;if(d?.id==='jackpot_bagua')for(const u of units(this,side,true))u.mana=u.maxMana}refreshHp(this,cur.side)}immortalCheck(this);for(const side of ['player','enemy'])rescueBySinhQi(this,side);const chosen=r?.unit;if(chosen&&(chosen.defeated||Number(chosen.hp)<=0)){if(this.state.current?.id===chosen.id)this.finishCurrent(true);return{type:'skip',unit:chosen,reason:'defeated-by-domain-tick'}}return r};
