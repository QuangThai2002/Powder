...
if(!process.argv.includes('--behavior-only')){
 ...
 const data=read('js/adventure-data.js'),map=read('js/adventure-map.js'),contentUi=read('js/combat-content-ui-v1890.js'),entry=read('js/combat-entry-v177.js'),controller=read('js/combat-result-controller-v1.js'),scene=read('src/game/combat2/scenes/BattleScene.ts');
 ...
 wire('Adventure UI blocks insufficient team',()=>{assert.match(map,/Không đủ Pow xuất trận\. Vui lòng chọn ít nhất 3 Pow\./);assert.match(map,/Cần ít nhất \$\{teamGate\.minimumTeamSize\} Pow để vào màn này/);assert.match(map,/reason:'minimum-team-size'/)});
 ...
 wire('Adventure copy is player-facing',()=>{for(const text of ['ĐIỀU KIỆN VÀO MÀN','Bạn đã đủ điều kiện để vào màn này','Câu hỏi trong trận sẽ lấy từ phần bạn đã học.','Màn này không yêu cầu bài học bắt buộc','Đội hình đã sẵn sàng'])assert.ok(map.includes(text),text);for(const text of ['ĐIỀU KIỆN CURRICULUM','Đã đủ điều kiện vào phó bản','Combat chỉ lấy câu từ các Learning Unit','Không có Learning Unit bắt buộc','Pow hợp lệ'])assert.ok(!map.includes(text),text)});
 wire('Adventure reward panel shows earned and received states',()=>{assert.match(map,/PHẦN THƯỞNG/);assert.match(map,/done\?'✓ Đã nhận phần thưởng':'Thắng trận để nhận phần thưởng'/);assert.match(map,/stage\.rewards\.coins/);assert.match(map,/stage\.rewards\.exp/)});
 wire('stage effects use player-facing copy',()=>{assert.match(contentUi,/HIỆU ỨNG MÀN CHƠI/);assert.match(contentUi,/Màn này không có hiệu ứng đặc biệt/);assert.doesNotMatch(contentUi,/ĐIỀU KIỆN CHIẾN TRƯỜNG|Không có modifier đặc biệt ở chặng này/i)});
}
