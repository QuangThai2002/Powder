...
async function fingerprintImage(cmd,url){
  ...
  return await evalJs(cmd,`(async()=>{const img=document.querySelector('img');if(!img)return null;await img.decode();const N=24,c=document.createElement('canvas');c.width=N;c.height=N;const x=c.getContext('2d',{willReadFrequently:true});x.drawImage(img,0,0,N,N);const d=x.getImageData(0,0,N,N).data,a=[];for(let i=0;i<d.length;i+=4)a.push((d[i]>>4),(d[i+1]>>4),(d[i+2]>>4));return{n:N,q:a,width:img.naturalWidth,height:img.naturalHeight}})()`);
}
function visualDiff(a,b){if(!a||!b||a.n!==b.n||a.q.length!==b.q.length)return{ratio:1,mean:1};let cells=a.q.length/3,bad=0,total=0;for(let i=0;i<a.q.length;i+=3){const d=Math.abs(a.q[i]-b.q[i])+Math.abs(a.q[i+1]-b.q[i+1])+Math.abs(a.q[i+2]-b.q[i+2]);total+=d;if(d>=7)bad++}return{ratio:bad/cells,mean:total/(cells*45)}}
const normalize=`(()=>{let s=document.getElementById('visual21008-freeze');if(!s){s=document.createElement('style');s.id='visual21008-freeze';s.textContent='*,*::before,*::after{animation:none!important;transition:none!important;caret-color:transparent!important}';document.head.appendChild(s)}const set=(q,t)=>{const e=document.querySelector(q);if(e)e.textContent=t};set('#bootProgressPct','50%');set('#bootStage','Đang mở thế giới Powder');set('#bootDetail','Chuẩn bị những khu vực bạn sắp khám phá...');set('#bootFiles','Đang hoàn thiện thế giới Powder...');set('#bootTip','Học mỗi ngày để phát triển Pow và mở thêm thử thách mới.');const f=document.querySelector('#bootProgressFill');if(f)f.style.width='50%';return{title:document.title,body:document.body?.innerText?.length||0,nodes:document.querySelectorAll('*').length}})()`;
...
if(updateBaseline){
  const baseline={version:'21.0.8',generatedFromRuntime:'21.0.4+21.0.7-boot-hotfix',static:report.static,screens:Object.fromEntries(Object.entries(report.screens).map(([k,v])=>[k,{fingerprint:v.fingerprint}])),runtime:report.runtime,thresholds:{visualRatio:{'index-desktop':0.12,'index-mobile':0.14,'admin-desktop':0.10},visualMean:0.055,jsGrowthRatio:1.03,cssGrowthRatio:1.03,manifestGrowthRatio:1.03}};
  ...
  if(!fs.existsSync(baselinePath)){report.errors.push('Missing visual/performance baseline');}
    ...
    for(const[name,s]of Object.entries(report.screens)){const d=visualDiff(s.fingerprint,b.screens?.[name]?.fingerprint);s.diff=d;check(`visual:${name}`,d.ratio<=(b.thresholds.visualRatio?.[name]??0.12)&&d.mean<=(b.thresholds.visualMean??0.055));const r=report.runtime[name],br=b.runtime?.[name]||{};check(`nodes:${name}`,r.Nodes<=(br.Nodes||r.Nodes)*1.15+150);check(`heap:${name}`,r.JSHeapUsedSize<=(br.JSHeapUsedSize||r.JSHeapUsedSize)*1.40+8388608);check(`layout:${name}`,r.LayoutCount<=(br.LayoutCount||r.LayoutCount)*2+25);check(`style:${name}`,r.RecalcStyleCount<=(br.RecalcStyleCount||r.RecalcStyleCount)*2+25)}
  }
}
