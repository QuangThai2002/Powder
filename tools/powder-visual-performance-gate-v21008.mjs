...
function visualDiff(a,b){const aq=fingerprintVector(a),bq=fingerprintVector(b);if(!a||!b||a.n!==b.n||aq.length!==bq.length||!aq.length)return{ratio:1,mean:1};let cells=aq.length/3,bad=0,total=0;for(let i=0;i<aq.length;i+=3){const d=Math.abs(aq[i]-bq[i])+Math.abs(aq[i+1]-bq[i+1])+Math.abs(aq[i+2]-bq[i+2]);total+=d;if(d>=7)bad++}return{ratio:bad/cells,mean:total/(cells*45)}}
