...
try{
  ...
  await sleep(1050);
  const reduced=await evaluate(cmd,`(()=>{const api=window.POWDER_COMBAT_SKILL_IDENTITY_V21018,m=document.getElementById('csi218Gate'),f=m.querySelector('.cv7-field');document.documentElement.classList.add('reduce-motion');const before=api.snapshot().phaseBeats;m.dataset.tf217Phase='charge';m.querySelector('.cv7-attack-flow-layer')?.remove();m.querySelector('.cv7-attack-callout')?.remove();const c=document.createElement('div');c.className='cv7-attack-callout player';c.innerHTML='<div><b>Pyrion</b><span>Viêm Tinh</span></div>';f.appendChild(c);m.querySelector('[data-cv7-skill="skill1"]').click();api.sync();const s=m.querySelector('.csi218-stage'),after=api.snapshot().phaseBeats;return{key:m.dataset.csi218Key,tier:s?.querySelector('[data-csi218-tier]')?.textContent||'',display:s?getComputedStyle(s).display:'none',target:s?.querySelector('[data-csi218-target]')?.textContent||'',before,after,classActive:document.documentElement.classList.contains('reduce-motion')}})()`);
  report.details.reducedMotion=reduced;report.checks.reducedMotion=reduced.classActive&&reduced.key==='skill1'&&reduced.tier==='KỸ NĂNG I'&&reduced.target==='Frostmaw'&&reduced.display!=='none'&&reduced.after===reduced.before;

  const telemetry=await evaluate(cmd,`window.POWDER_COMBAT_SKILL_IDENTITY_V21018.snapshot()`);const delta={stageCreates:telemetry.stageCreates-baseline.stageCreates,classifications:telemetry.classifications-baseline.classifications,inputHints:telemetry.inputHints-baseline.inputHints,phaseBeats:telemetry.phaseBeats-baseline.phaseBeats,ultimateBeats:telemetry.ultimateBeats-baseline.ultimateBeats,impactLatches:telemetry.impactLatches-baseline.impactLatches,pressureReductions:telemetry.pressureReductions-baseline.pressureReductions,keys:Object.fromEntries(Object.keys(telemetry.keys).map(k=>[k,(telemetry.keys[k]||0)-(baseline.keys?.[k]||0)]))};report.details.telemetry=telemetry;report.details.telemetryDelta=delta;report.checks.telemetry=delta.stageCreates===1&&delta.classifications>=10&&delta.inputHints>=6&&delta.phaseBeats>=8&&delta.ultimateBeats>=2&&delta.impactLatches>=1&&delta.pressureReductions>=1&&['basic','skill1','skill2','exclusive','ultimate'].every(k=>delta.keys[k]>=2);
