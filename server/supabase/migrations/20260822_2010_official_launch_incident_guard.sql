-- Powder 20.1.0 · Official Launch Incident Guard
create or replace function public.powder_official_launch_preflight_v2000(p_channel text default 'production'::text, p_build text default 'powder-20.0.0-official-launch-gate'::text, p_manifest_hash text default ''::text)
returns jsonb language plpgsql security definer set search_path='public' as $function$
declare
  c text := case when p_channel='staging' then 'staging' else 'production' end;
  b public.release_builds; ch public.release_channels; ro public.release_rollout_v1950; integ jsonb := '{}'::jsonb;
  ...
  rollback_ok boolean:=false; candidate_ok boolean:=false; integrity_ok boolean:=false; pilot_ok boolean:=false; rollout_ok boolean:=false; evidence_ok boolean:=false; incident_ok boolean:=false; ready boolean:=false;
begin
  ...
  select count(distinct d.user_id),coalesce(sum(d.pvp_matches),0),coalesce(sum(d.crash_count),0),count(*) filter(where d.avg_fps>0 and d.avg_fps<30),count(*) filter(where coalesce((d.last_summary->>'savePass')::boolean,true)=false)
    ...
    from public.pilot_device_health_v1970 d join public.pilot_cohort_v1970 c0 on c0.user_id=d.user_id and c0.active=true
   where d.last_seen_at>=now()-interval '7 days';
  select count(*) filter(where i.severity='critical'),count(*) filter(where i.severity='high') into critical,high
    from public.pilot_issue_reports_v1970 i join public.pilot_cohort_v1970 c0 on c0.user_id=i.user_id and c0.active=true
   where i.status in ('open','triaged');
  select count(*) filter(where severity='critical'),count(*) filter(where severity='high') into ops_critical,ops_high
  ...
  select count(*) into evidence_pass from public.official_launch_evidence_v2000 e
   where e.channel=c and e.build_id=p_build and e.status='pass'
     ...
     and (p_manifest_hash='' or e.manifest_hash=p_manifest_hash) and e.evidence_sha256 ~ '^[0-9a-fA-F]{64}$';
  ...
  ready := candidate_ok and integrity_ok and pilot_ok and rollout_ok and evidence_ok and rollback_ok and incident_ok;
  return jsonb_build_object('version','20.0.0','channel',c,'buildId',p_build,'manifestHash',p_manifest_hash,'ready',ready,
    'checks',jsonb_build_object('candidate',candidate_ok,'integrity',integrity_ok,'realPilot',pilot_ok,'rolloutNormal',rollout_ok,'evidence4of4',evidence_ok,'rollbackTarget',rollback_ok,'liveOpsIncidents',incident_ok),
