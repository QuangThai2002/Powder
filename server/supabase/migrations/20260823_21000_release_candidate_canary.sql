-- Powder 21.0.0 · Official Production candidate evidence and staged rollout policy
begin;

create table if not exists public.release_candidate_config_v21000(
...
insert into public.release_candidate_config_v21000(id) values(1) on conflict(id) do nothing;

create table if not exists public.release_candidate_evidence_v21000(
...
create index if not exists release_candidate_evidence_v21000_build_idx on public.release_candidate_evidence_v21000(channel,build_id,created_at desc);

create table if not exists public.release_canary_state_v21000(
...
create table if not exists public.release_canary_stage_evidence_v21000(
  channel text not null,build_id text not null,manifest_hash text not null,artifact_sha256 text not null,
...
create table if not exists public.release_canary_rollback_drills_v21000(
...
create index if not exists release_canary_rollback_drills_v21000_build_idx on public.release_canary_rollback_drills_v21000(channel,source_build,created_at desc);

create or replace function public.powder_release_candidate_record_v21000(
...
begin
 if p_build<>'powder-21.0.0-official-production' then raise exception 'RC21000_BUILD_MISMATCH';end if;
 if p_manifest_hash!~'^[0-9a-fA-F]{16,64}$' or p_artifact_sha256!~'^[0-9a-fA-F]{64}$' or p_evidence_sha256!~'^[0-9a-fA-F]{64}$' then raise exception 'RC21000_HASH_INVALID';end if;
 ...
 insert into public.release_candidate_evidence_v21000(channel,build_id,manifest_hash,artifact_sha256,automated_checks,manual_checks,critical_count,high_count,automated_pass,manual_pass,pass,evidence_sha256,runner_revision,created_by)
...
create or replace function public.powder_canary_rollback_drill_record_v21000(
...
begin
 if p_source_build<>'powder-21.0.0-official-production' or p_target_build='' or p_target_build=p_source_build then raise exception 'RC21000_ROLLBACK_TARGET_INVALID';end if;
 if p_manifest_hash!~'^[0-9a-fA-F]{16,64}$' or p_artifact_sha256!~'^[0-9a-fA-F]{64}$' or p_evidence_sha256!~'^[0-9a-fA-F]{64}$' then raise exception 'RC21000_ROLLBACK_HASH_INVALID';end if;
 if not exists(select 1 from public.release_builds where channel=c and build_id=p_target_build and checksum~'^[0-9a-fA-F]{64}$') then raise exception 'RC21000_ROLLBACK_BUILD_UNKNOWN';end if;
 ...
 insert into public.release_canary_rollback_drills_v21000(channel,source_build,target_build,artifact_sha256,manifest_hash,duration_seconds,data_integrity_pass,post_rollback_health_pass,pass,evidence_sha256,runner_revision,created_by)
...
create or replace function public.powder_release_candidate_posture_v21000(p_channel text default 'production',p_build text default 'powder-21.0.0-official-production',p_manifest_hash text default '')
returns jsonb language plpgsql security definer set search_path=public as $function$
declare c text:=case when p_channel='staging' then 'staging' else 'production' end;cfg public.release_candidate_config_v21000;e public.release_candidate_evidence_v21000;d public.release_canary_rollback_drills_v21000;a public.official_release_authorization_v2070;fresh boolean:=false;drill_fresh boolean:=false;ready boolean:=false;
begin
 select * into cfg from public.release_candidate_config_v21000 where id=1;
 select * into e from public.release_candidate_evidence_v21000 where channel=c and build_id=p_build and (p_manifest_hash='' or manifest_hash=lower(p_manifest_hash)) order by created_at desc limit 1;
 select * into a from public.official_release_authorization_v2070 where channel=c and build_id=p_build;
 select * into d from public.release_canary_rollback_drills_v21000 where channel=c and source_build=p_build and (a.rollback_target_build is null or target_build=a.rollback_target_build) and (p_manifest_hash='' or manifest_hash=lower(p_manifest_hash)) order by created_at desc limit 1;
 ...
 return jsonb_build_object('version','21.0.0','ready',ready,'armed',coalesce(cfg.enabled,false),'fresh',fresh,'rollbackDrillFresh',drill_fresh,'evidence',case when e.id is null then null else to_jsonb(e) end,'rollbackDrill',case when d.id is null then null else to_jsonb(d) end,'checks',jsonb_build_object('automatedRC',coalesce(e.automated_pass,false),'manualSmoke13',coalesce(e.manual_pass,false),'criticalZero',coalesce(e.critical_count,1)=0,'highZero',coalesce(e.high_count,1)=0,'manifestBound',e.id is not null and (p_manifest_hash='' or e.manifest_hash=lower(p_manifest_hash)),'rollbackDrill',coalesce(d.pass,false)),'checkedAt',now());
end $function$;

create or replace function public.powder_canary_stage_health_v21000(p_channel text,p_build text,p_manifest_hash text,p_rollout_percent int)
returns jsonb language plpgsql security definer set search_path=public as $function$
declare c text:=case when p_channel='staging' then 'staging' else 'production' end;pct int:=p_rollout_percent;window_m int;min_req bigint;min_span numeric;max_err numeric;max_p95 numeric;max_p99 numeric;h jsonb;s public.release_canary_state_v21000;b public.release_builds;a public.official_release_authorization_v2070;elapsed numeric:=0;pass boolean:=false;
begin
 if pct not in(5,20,50,100) then raise exception 'RC21000_STAGE_INVALID';end if;
 ...
 select * into s from public.release_canary_state_v21000 where channel=c and build_id=p_build;
 select * into b from public.release_builds where channel=c and build_id=p_build;
 select * into a from public.official_release_authorization_v2070 where channel=c and build_id=p_build;
 if s.build_id is null or s.rollout_percent<>pct then raise exception 'RC21000_STAGE_NOT_ACTIVE';end if;
 if lower(s.manifest_hash)<>lower(p_manifest_hash) or lower(s.artifact_sha256)<>lower(coalesce(b.checksum,'')) or lower(a.manifest_hash)<>lower(p_manifest_hash) or lower(a.artifact_checksum)<>lower(coalesce(b.checksum,'')) then raise exception 'RC21000_STAGE_BUILD_DRIFT';end if;
 ...
 return jsonb_build_object('version','21.0.0','channel',c,'buildId',p_build,'manifestHash',lower(p_manifest_hash),'artifactSha256',lower(coalesce(b.checksum,'')),'rolloutPercent',pct,'stageElapsedMinutes',round(elapsed,1),'pass',pass,'health',h,'policy',jsonb_build_object('windowMinutes',window_m,'minRequests',min_req,'minObservedMinutes',min_span,'maxErrorRate',max_err,'maxP95Ms',max_p95,'maxP99Ms',max_p99,'maxCrashes',0,'maxSaveConflicts',0),'checkedAt',now());
end $function$;

create or replace function public.powder_capture_canary_stage_health_v21000(p_channel text,p_build text,p_manifest_hash text,p_rollout_percent int)
returns jsonb language plpgsql security definer set search_path=public as $function$
declare c text:=case when p_channel='staging' then 'staging' else 'production' end;r jsonb;st text;
begin
 r:=public.powder_canary_stage_health_v21000(c,p_build,p_manifest_hash,p_rollout_percent);st:=case when coalesce((r->>'pass')::boolean,false) then 'pass' else 'hold' end;
 insert into public.release_canary_stage_evidence_v21000(channel,build_id,manifest_hash,artifact_sha256,rollout_percent,status,health,policy,checked_at,checked_by)
...
alter table public.release_candidate_config_v21000 enable row level security;
alter table public.release_candidate_evidence_v21000 enable row level security;
alter table public.release_canary_state_v21000 enable row level security;
alter table public.release_canary_stage_evidence_v21000 enable row level security;
alter table public.release_canary_rollback_drills_v21000 enable row level security;
revoke all on public.release_candidate_config_v21000,public.release_candidate_evidence_v21000,public.release_canary_state_v21000,public.release_canary_stage_evidence_v21000,public.release_canary_rollback_drills_v21000 from public,anon,authenticated;
grant select,insert,update,delete on public.release_candidate_config_v21000,public.release_candidate_evidence_v21000,public.release_canary_state_v21000,public.release_canary_stage_evidence_v21000,public.release_canary_rollback_drills_v21000 to service_role;
grant usage,select on sequence public.release_candidate_evidence_v21000_id_seq,public.release_canary_rollback_drills_v21000_id_seq to service_role;
revoke all on function public.powder_release_candidate_record_v21000(text,text,text,text,jsonb,jsonb,int,int,text,text,text),public.powder_canary_rollback_drill_record_v21000(text,text,text,text,text,int,boolean,boolean,text,text,text),public.powder_release_candidate_posture_v21000(text,text,text),public.powder_canary_stage_health_v21000(text,text,text,int),public.powder_capture_canary_stage_health_v21000(text,text,text,int) from public,anon,authenticated;
grant execute on function public.powder_release_candidate_record_v21000(text,text,text,text,jsonb,jsonb,int,int,text,text,text),public.powder_canary_rollback_drill_record_v21000(text,text,text,text,text,int,boolean,boolean,text,text,text),public.powder_release_candidate_posture_v21000(text,text,text),public.powder_canary_stage_health_v21000(text,text,text,int),public.powder_capture_canary_stage_health_v21000(text,text,text,int) to service_role;
